import { BackSide, Box2, CanvasTexture, CircleBufferGeometry, Color, CylinderBufferGeometry, DoubleSide, Euler, EventDispatcher, FontLoader, FrontSide, Frustum, Group, MOUSE, Mesh, MeshBasicMaterial, MeshLambertMaterial, PerspectiveCamera, PlaneBufferGeometry, PointLight, Quaternion, Raycaster, RingBufferGeometry, Scene, SphereBufferGeometry, Spherical, TextureLoader, Vector2, Vector3, WebGLRenderer } from 'three';
import { getWebGLErrorMessage, webgl } from 'Detector';
import { dopri, uncmin } from 'numericjs';
...
var Utils = function () {
	...
	createClass(Utils, null, [{
		...
		value: function quaternionToEuler(q, order) {
			order = order || 'XYZ';
			var euler = new Euler(0, 0, 0, order);
		...
		value: function getQuaternion(ctr, start, end, q) {
			var v1 = new Vector3().subVectors(start, ctr);
			v1.normalize();
			var v2 = new Vector3().subVectors(end, ctr);
			v2.normalize();
			q = q || new Quaternion();
		...
		value: function printWorldOrientationAndPosition(obj, name) {
			...
			var ws = new Spherical().setFromVector3(yaxis);
			var msg = name ? name + ': ' : '';
...
var starGeom = new CircleBufferGeometry(unit, 32);
var DecoratedStar = function (_Star) {
	...
	function DecoratedStar(starData, zoom, colorLevel, baseBrightness, baseRadius) {
		...
		_this.mesh = new Mesh(starGeom, material);
		_this.mesh.star = _this;
		var theta = _this.longitude * Math.PI / 180;
		var phi = (90 - _this.latitude) * Math.PI / 180;
		_this.spherical = new Spherical(distance, phi, Math.PI / 2 + theta);
...
var stmp = new Spherical();
var planetGeom = new SphereBufferGeometry(1, 180, 180);
var DecoratedPlanet = function () {
	...
	createClass(DecoratedPlanet, [{
		...
		value: function createRingMesh() {
			var ringGeom = new RingBufferGeometry(this.planet.rings.innerRadius * unit, this.planet.rings.outerRadius * unit, 180, 1);
			...
			var mesh = new Mesh(ringGeom, material);
	...
	createClass(Bands, [{
		...
		value: function _createBands() {
			...
			for (var idx = 1; idx < 18; idx++) {
				...
				band = this._createBand(radius * Math.sin(phi), 0, 0, idx == 9 ? specialBandMaterial : bandMaterial, new Vector3(0, radius * Math.cos(phi), 0));
...
var v2tmp1 = new Vector2();
var v2tmp2 = new Vector2();
var v3tmp1 = new Vector3();
	...
	createClass(Geometry, null, [{
		...
		value: function pushFunctionLine(line1, r1, r2) {
			...
			return function (line2) {
				...
				var delta = new Vector3().subVectors(line2.end, start);
				var u = new Vector3();
				var v = new Vector3();
				function coeff(t) {
					u.copy(delta);
...
function getTextureData(labels, fontSize, fontStyle, opacity, colorIndex) {
	...
	if (!material) {
		var texture = textureData.texture;
		var color = new Color(colors[colorIndex]);
		var _material = new MeshBasicMaterial({ alphaMap: texture,
			transparent: true,
			color: color,
			side: FrontSide });
	...
	createClass(StarLabel, [{
		...
		value: function intersectsDisk(center, radius) {
			...
			if (this.isCircle) {
				...
				var c = new Vector2(v3tmp$2.dot(this.vtr), v3tmp$2.dot(this.orthogVtr));
		...
		value: function getGlobalRectangle(p1, p2, p3, p4) {
			...
			var tmp1 = new Vector3().copy(this.vtr).multiplyScalar(mesh.width / 2);
			var tmp2 = new Vector3().copy(this.orthogVtr).multiplyScalar(mesh.height / 2);
...
var LabeledStar = function (_DecoratedStar) {
	...
	createClass(LabeledStar, [{
		...
		value: function computeLabelVectors(shortName) {
			...
			var xVtr = new Vector3();
			var yVtr = new Vector3();
...
var v3tmp$3 = new Vector3();
var v3tmp2 = new Vector3();
			...
			for (var cstl in constellations) {
				if (cstl !== '' && constellations.hasOwnProperty(cstl)) {
					...
					for (var idx = 0; idx < constell.labelPos.length; idx++) {
						...
						var spherical = new Spherical(distance * 1.0002, radians(90 - labelPos.latitude), radians(90 + labelPos.longitude));
...
var disablePPointerEvents = false;
...
function init(window, isMobile, isApple) {
	...
	var renderer = new WebGLRenderer({ antialias: true, logarithmicDepthBuffer: true });
	renderer.setClearColor(new Color(0x000000, 1.0), 0);
	...
	group.name = 'group';
	scene.add(group);
	var solarSystem = new Group();
	solarSystem.name = 'solarSystem';
	group.add(solarSystem);
	var pointLight = new PointLight(0xffffff, ssettings.get('planets', 'sunbrightness'), distance - unit, 2);
	...
	controls = new Controls(viewer$$1.camera, renderer);
	...
	function getPositionInSolarSystem(obj) {
		var local = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : new Vector3();
		obj.parent.updateMatrixWorld();
...
export { init, ssettings, SettingsControl, controls, refreshObj, disableMouseEvents, disablePPointerEvents, setTrueNorth, setDirectionFromCompass, setNoDOC, doOrient, absoluteOrientationHandler, deviceOrientationHandler, dismissContextMenu };
